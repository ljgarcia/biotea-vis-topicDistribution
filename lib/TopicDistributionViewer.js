...
var createTooltip = function(viewer, datum) {
    var prefixedId = viewer.options.prefixID ? viewer.options.prefixID + ':' + datum.id : datum.id;
    if (datum.score) {
        Tooltip.create(d3.select(viewer.options.el), 'Article',
            [['Id', prefixedId], ['Title', datum.display], ['Score', datum.score.toFixed(3)]]);
    } else {
        Tooltip.create(d3.select(viewer.options.el), 'Article',
            [['Id', prefixedId], ['Title', datum.display]]);
    }
};

var initMatrix = function(viewer, data) {
...
var TopicDistributionViewer = function(opts){
    ...
    viewer.loadURL = function() {
        ...
        _.each(viewer.options.ids, function(id, index) {
            ...
            loader.done(function(loadedData) {
                initMatrix(viewer, loadedData.data);
            }).fail( function(e) {
                console.log(e);
            });
        });
    ...
    viewer.loadData = function(dataArray) {
        cleanView(viewer);
        _.each(dataArray, function(data) {
            initMatrix(viewer, data);
        });
    };
    ...
    viewer.setData = function(dataArray) {
