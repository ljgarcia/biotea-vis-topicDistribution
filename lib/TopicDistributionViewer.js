...
var TopicDistributionViewer = function(opts){
    ...
    var margin = {left: 40, bottom: 40};
    var gridSizeX = Math.floor((viewer.options.width-margin.left) / viewer.options.ids.length);
    var gridSizeY = Math.floor((viewer.options.height-margin.bottom) / viewer.topics.length);
    ...
    viewer.load = function() {
        _.each(viewer.options.ids, function(id) {
                ...
                loader.done(function(loadedData) {
                    var cards = viewer.svg.append('g').attr('transform', 'translate(' + margin.left + ', 0)')
                        .selectAll('.score')
                        .data(loadedData.data).enter().append('rect');
                    cards.attr('x', function(d) {
                            return _.indexOf(viewer.options.ids, d.id) * gridSizeX;
                        })
                        .attr('y', function(d) {
                            return _.indexOf(viewer.topics, d.group) * gridSizeY;
                        })
                        .attr('class', 'score')
    ...
    viewer.render = function() {
        if (viewer.options.path && viewer.options.ids) {
            displayTopics(viewer, gridSizeY);
            displayLegend(viewer, margin);
            viewer.load();
        }
    };
