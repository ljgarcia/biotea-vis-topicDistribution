...
var createTooltip = function(viewer, dist, datum) {
    ...
    if (datum.score) {
        Tooltip.create(d3.select(viewer.options.el), 'Article',
            [['Id', prefixedId], ['Title', dist.display], ['Score', datum.score.toFixed(10)],
            ['Average tf-idf', datum.avgTfIdf.toFixed(10)]]);
    } else {
        Tooltip.create(d3.select(viewer.options.el), 'Article',
    ...
    var cards = viewer.svg.append('g')
        ...
        .on('click', function(d) {
            ...
            viewer.parser.getDispatcher().selected({
                ...
                article: dist.id,
                group: d.group,
                score: d.score ? d.score.toFixed(10) : d.score,
                avgTfIdf: d.avgTfIdf ? d.avgTfIdf.toFixed(10) : d.avgTfIdf
            });
        });
    ;
};
