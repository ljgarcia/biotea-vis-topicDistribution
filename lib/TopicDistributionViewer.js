...
var addShadow = function(viewer) {
    viewer.selection = viewer.svg.append('g').attr('class', 'biotea_score_shadow_group');
...
var TopicDistributionViewer = function(opts){
    ...
    viewer.load = function() {
        ...
        d3.select(viewer.options.el).selectAll('.biotea_score_shadow_group').remove();
        viewer.selection = undefined;
        _.each(viewer.options.ids, function(id, index) {
            var loader = RDFLoader.get(viewer.options.path, id, viewer.options.display[index]);
            loader.done(function(loadedData) {
                var cards = viewer.svg.append('g')
                    ...
                    .style('fill', function(d) {
                        return viewer.useColorArray ? viewer.options.colorArray[Math.floor(d.score * 10)]
                            : viewer.colorScale(d.score);
                    })
                    .attr('title', function(d) { return 'Article: ' + d.display + ', score: ' + d.score.toFixed(3); })
                    .on('click', function(d) {
                        if (!viewer.selection) {
                            addShadow(viewer);
                        }
